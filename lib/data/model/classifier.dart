...
import 'package:tflite_flutter_helper_plus/tflite_flutter_helper_plus.dart' as tfLite;
...
class Classifier {
  ...
  tfLite.ImageProcessor? imageProcessor;
  ...
  /// image pre process
  tfLite.TensorImage getProcessedImage(tfLite.TensorImage inputImage) {
    final padSize = max(inputImage.height, inputImage.width);

    imageProcessor ??= tfLite.ImageProcessorBuilder()
        .add(
      tfLite.ResizeWithCropOrPadOp(
        padSize,
        padSize,
      ),
    )
  ...
  List<Recognition> predict(image_lib.Image image) {
    ...
    var inputImage = tfLite.TensorImage.fromImage(image);
    ...
    var normalizedTensorBuffer = tfLite.TensorBufferFloat.dynamic();
    ...
    final outputLocations = tfLite.TensorBufferFloat(_outputShapes[0]);
    final outputs = {
